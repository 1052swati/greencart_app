import React, { useEffect, useState } from 'react';
import { assets } from '../assets/assets';
import { useAppContext } from '../Context/AppContext';
import { useNavigate } from 'react-router-dom';
import toast from 'react-hot-toast';

// Input Field component
const InputField = ({ type, placeholder, name, handleChange, value }) => (
    <input
        className='w-full px-2 py-2.5 border border-gray-500/30 rounded outline-none text-gray-500 focus:border-primary transition'
        type={type}
        placeholder={placeholder}
        name={name}
        value={value}
        onChange={handleChange}
        required
    />
);

const AddAddress = () => {

    const { setAddress, axios, user } = useAppContext();
    const navigate = useNavigate();

    const [addressLocal, setAddressLocal] = useState({
        firstName: '',
        lastName: '',
        email: '',
        street: '',
        city: '',
        state: '',
        zipcode: '',
        country: '',
        phone: '',
    });

    // ✅ useEffect should be here (top level)
    useEffect(() => {
        if (!user) {
            navigate('/cart');
        }
    }, [user, navigate]);

    const handleChange = (e) => {
        const { name, value } = e.target;
        setAddressLocal(prev => ({
            ...prev,
            [name]: value
        }));
    };

    // ✅ make async
    const onSubmitHandler = async (e) => {
        e.preventDefault();

        try {
            const { data } = await axios.post(
                '/api/address/add',
                { address: addressLocal },
               { withCredentials: true }
            );

            if (data.success) {
                toast.success(data.message);
                setAddress(addressLocal);
                navigate('/cart');
            } else {
                toast.error(data.message);
            }

        } catch (error) {
            console.log(error)
            toast.error("Address not added");
        }
    };

    return (
        <div className='mt-16 pb-16'>
            <p className='text-2xl md:text-3xl text-gray-500'>
                Add Shipping <span className='font-semibold text-[var(--color-primary)]'>Address</span>
            </p>

            <div className='flex flex-col-reverse md:flex-row justify-between mt-10'>
                <div className='flex-1 max-w-md'>
                    <form onSubmit={onSubmitHandler} className='space-y-3 mt-6 text-sm'>

                        <div className='grid grid-cols-2 gap-4'>
                            <InputField
                                handleChange={handleChange}
                                value={addressLocal.firstName}
                                name='firstName'
                                type='text'
                                placeholder='First Name'
                            />
                            <InputField
                                handleChange={handleChange}
                                value={addressLocal.lastName}
                                name='lastName'
                                type='text'
                                placeholder='Last Name'
                            />
                        </div>

                        <InputField
                            handleChange={handleChange}
                            value={addressLocal.email}
                            name='email'
                            type='email'
                            placeholder='Email address'
                        />

                        <InputField
                            handleChange={handleChange}
                            value={addressLocal.street}
                            name='street'
                            type='text'
                            placeholder='Street'
                        />

                        <div className='grid grid-cols-2 gap-4'>
                            <InputField
                                handleChange={handleChange}
                                value={addressLocal.city}
                                name='city'
                                type='text'
                                placeholder='City'
                            />
                            <InputField
                                handleChange={handleChange}
                                value={addressLocal.state}
                                name='state'
                                type='text'
                                placeholder='State'
                            />
                        </div>

                        <div className='grid grid-cols-2 gap-4'>
                            <InputField
                                handleChange={handleChange}
                                value={addressLocal.zipcode}
                                name='zipcode'
                                type='number'
                                placeholder='Zip code'
                            />
                            <InputField
                                handleChange={handleChange}
                                value={addressLocal.country}
                                name='country'
                                type='text'
                                placeholder='Country'
                            />
                        </div>

                        <InputField
                            handleChange={handleChange}
                            value={addressLocal.phone}
                            name='phone'
                            type='text'
                            placeholder='Phone'
                        />

                        <button className='w-full mt-6 bg-[var(--color-primary)] text-white py-3 hover:bg-primary-dull transition cursor-pointer uppercase'>
                            Save address
                        </button>
                    </form>
                </div>

                <img
                    className='md:mr-16 mb-16 md:mt-0 '
                    src={assets.add_address_image}
                    alt="Add Address"
                />
            </div>
        </div>
    );
};

export default AddAddress;